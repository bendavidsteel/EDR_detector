function SCR = findpeaks2(data, times, offset, start_WT, end_WT, thres_low, theta, alpha)
...
% 		max_deriv:           list of floats, max derivative within 1 second of apex of SCR
    ...
    peak_times = zeros(length(data)-1,1);
    peak_eda = zeros(length(data)-1,1);
	...
	max_deriv = zeros(length(data)-1,1);
	rise_time = zeros(length(data)-1,1);
	...
	amplitude = zeros(length(data)-1,1);
